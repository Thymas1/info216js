(function (window, undefined) {
    "use strict";
    var document = window.document, S = {}, globalGetSet, globalDefaultsQuery, globalDefaultsChart;
    S.core = function () {
        var VERSION = "0.6.0", HOMEPAGE = "http://mgskjaeveland.github.io/sgvizler/",
        ...
        return {VERSION: VERSION, HOMEPAGE: HOMEPAGE, LOGOIMAGE: LOGOIMAGE, CHARTSCSS: CHARTSCSS}
    }();
    S.util = function () {
        var isString = function (input) {
            ...
            }, isFunction = $.isFunction, isArray = $.isArray,
            URLpattern = new RegExp("^(https?:\\/\\/)?" + "((([a-z\\d]([a-z\\d-]*[a-z\\d])*)\\.)+[a-z]{2,}|" + "((\\d{1,3}\\.){3}\\d{1,3}))" + "(\\:\\d+)?(\\/[-a-z\\d%_.~+]*)*" + "(\\?[;&a-z\\d%_.~+=-]*)?" + "(\\#[-a-z\\d_]*)?$", "i"),
            isURL = function (input) {
                ...
                var i, len, segments = path.split("."), cursor = object || window;
                for (i = 0, len = segments.length; i < len; i += 1) {
                    if (cursor !== undefined && cursor[segments[i]] === undefined && create) {
                        cursor[segments[i]] = {}
                    }
                    cursor = cursor && cursor[segments[i]]
                }
                return cursor
                ...
                var i, len, element = $(document.createElement(elementType)), attr, childs = toArray(children), child;
    ...
    S.namespace = function () {
        var startsWith = S.util.startsWith, isString = S.util.isString, nss = {
        ...
        return {
            get: function (prefix) {
                ...
                if (isString(url)) {
                    for (prefix in nss) {
                        if (nss.hasOwnProperty(prefix) && startsWith(url, nss[prefix])) {
                ...
                if (isString(qname)) {
                    for (prefix in nss) {
                        if (nss.hasOwnProperty(prefix) && startsWith(qname, prefix + ":")) {
    ...
    S.registry = function () {
        var util = S.util, GVIZ = "google.visualization", DATATABLE = GVIZ + ".DataTable", GMAP = "google.maps",
            modules = {chart: [GVIZ], datatable: []}, registry = {
                google: {
                    visualization: {
                        DataTable: {t: "core", d: {i: GVIZ}},
                        LineChart: {d: {i: "corechart"}},
                        AreaChart: {d: {i: "corechart"}},
                        SteppedAreaChart: {d: {i: "corechart"}},
                        PieChart: {d: {i: "corechart"}},
                        BubbleChart: {d: {i: "corechart"}},
                        ColumnChart: {d: {i: "corechart"}},
                        BarChart: {d: {i: "corechart"}},
                        ImageSparkLine: {d: {i: "imagesparkline"}},
                        ScatterChart: {d: {i: "corechart"}},
                        CandlestickChart: {d: {i: "corechart"}},
                        Gauge: {d: {i: "gauge"}},
                        OrgChart: {d: {i: "orgchart"}},
                        TreeMap: {d: {i: "treemap"}},
                        AnnotatedTimeLine: {d: {i: "annotatedtimeline"}},
                        MotionChart: {d: {i: "motionchart"}},
                        GeoChart: {d: {i: "geochart"}},
                        GeoMap: {d: {i: "geomap"}},
                        Map: {d: {i: "map"}},
                        Table: {d: {i: "table"}}
                    }, maps: {Map: {d: {i: "map"}}}
        ...
        return {
            GVIZ: GVIZ, GMAP: GMAP, DATATABLE: DATATABLE, chartModules: function () {
                return modules.chart
            }, chartFunctions: function () {
                var i, len, libs = modules.chart, lib, func, charts = [];
                for (i = 0, len = libs.length; i < len; i += 1) {
                    lib = util.getObjectByPath(libs[i], registry);
                    for (func in lib) {
                        if (lib.hasOwnProperty(func) && (lib[func].t === undefined || lib[func].t === "chart")) {
                ...
                var regFunc = util.getObjectByPath(functionName, registry), deps = regFunc && regFunc.d || {};
                if (deps.i) {
    ...
    S.logger = function () {
        var startTime = Date.now(), elapsedTime = function () {
    ...
    S.charts = function () {
        var inherit = S.util.inherit, addFunction = S.registry.addFunction, Chart,
    ...
    S.datatables = function () {
        var addFunction = S.registry.addFunction, add = function (module, name, func, dependencies) {
    ...
    S.parser = function () {
        var namespace = S.namespace, datatypeXSD2JS = function () {
            var xsdns = namespace.get("xsd"), table = [];
            ...
            if (gdatatype === "number") {
                ...
                newvalue = new Date(value.substr(0, 4), value.substr(5, 2), value.substr(8, 2))
            } else if (gdatatype === "datetime") {
                newvalue = new Date(value.substr(0, 4), value.substr(5, 2), value.substr(8, 2), value.substr(11, 2), value.substr(14, 2), value.substr(17, 2))
            } else if (gdatatype === "timeofday") {
                newvalue = [value.substr(0, 2), value.substr(3, 2), value.substr(6, 2)]
            } else {
                if (stype === "uri") {
        ...
        return {
            convertXML: function (sxml) {
                ...
                $(sresults).each(function () {
                    var gvalue, scells, scell, stype, svalue, grow = [];
                    for (c = 0, clen = gcols.length; c < clen; c += 1) {
                        gvalue = null;
                        scells = $(this).find('binding[name="' + gcols[c].id + '"]');
                        if (scells.length && $(scells).first().children().first() && $(scells).first().children().first()[0]) {
                            scell = $(scells).first().children().first()[0];
                ...
                var c, clen, r, rlen, srow, grow, gvalue, sdatatype, gcols = [], grows = [], gdatatype = [],
    ...
    S.loader = function () {
        var util = S.util, logger = S.logger, registry = S.registry, moduleGooVis = registry.GVIZ,
            moduleGooMap = registry.GMAP, loaders = {}, loadGVizScript = function (packages, loadLib) {
                var loader, promise = {}, packs = util.removeDuplicates(packages).sort(), options;
                ...
                var i, ilen, func, deferreds = [], gVizPacks = [], gLoader, deps, dep, loadGLib;
                        ...
                        if (deps.hasOwnProperty(dep)) {
                            if (loaders[deps[dep]]) {
                                ...
                                if (util.startsWith(dep, moduleGooVis)) {
                                    ...
                                    loaders[deps[dep]] = $.getScript(deps[dep]).done(function () {
                                        logger.log("loadDependencies: loaded: " + deps[dep])
                                    });
                                    deferreds.push(loaders[deps[dep]]);
                                    logger.log("loadDependencies: loading script: " + deps[dep])
                                }
    ...
    S.defaults = function () {
        var moduleGooVis = S.registry.GVIZ, queryDefaults = {
                query: "SELECT ?class (count(?instance) AS ?noOfInstances)\n" + "WHERE{ ?instance a ?class }\n" + "GROUP BY ?class\n" + "ORDER BY ?class",
                froms: [],
                endpoint: "http://sws.ifi.uio.no/sparql/world",
                endpoint_output_format: "json",
                endpoint_results_urlpart: "?output=text&amp;query=",
                validator_url: "http://sparql.org/validate/query" + "?languageSyntax=SPARQL" + "&amp;outputFormat=sparql" + "&amp;linenumbers=true" + "&amp;query=",
                chart: moduleGooVis + ".Table",
                loglevel: 2
            }, chartDefaults = {width: 800, height: 400, chartArea: {left: "5%", top: "5%", width: "75%", height: "80%"}},
            chartSpecificDefaults = function () {
                var options = {};
                options[moduleGooVis + ".GeoMap"] = {dataMode: "markers"};
                options[moduleGooVis + ".Map"] = {dataMode: "markers"};
                options[moduleGooVis + ".Sparkline"] = {showAxisLines: false};
                return options
            }();
        return {
            query: queryDefaults, chart: chartDefaults, getQueryOptions: function () {
    ...
    S.Query = function (queryOpt, chartOpt) {
        var util = S.util, getset = util.getset, prefixesSPARQL = S.namespace.prefixesSPARQL, registry = S.registry,
            moduleGooVis = registry.GVIZ, fDataTable = registry.DATATABLE, parser = S.parser,
            loadDependencies = S.loader.loadDependencies, logger = S.logger, defaults = S.defaults, qfXML = "xml",
            qfJSON = "json", qfJSONP = "jsonp", queryOptions, chartOptions, listeners = {}, dataTable, queryResult,
            noOfResults, addFrom = function (url) {
                ...
                var i, len = queryOptions.froms.length, from;
                if (len) {
                    from = "";
                    for (i = 0; i < len; i += 1) {
                ...
                var promise, myEndpointOutput = endpointOutputFormat();
                insertFrom();
                if (myEndpointOutput !== qfJSONP && window.XDomainRequest) {
                    promise = function () {
                        var qx = $.Deferred(), xdr = new XDomainRequest,
                            url = endpointURL() + "?query=" + getEncodedQuery() + "&output=" + myEndpointOutput;
                        xdr.open("GET", url);
                        xdr.onload = function () {
                            var data;
                ...
                if (queryResult !== undefined) {
                    ...
                    qr = sendQuery();
                    qr.fail(function (xhr, textStatus, thrownError) {
                ...
                var qdt, myDatatableFunction = datatableFunction();
                if (dataTable) {
                    qdt = dataTable
                } else {
                    qdt = $.when(saveQueryResults(), loadDependencies(fDataTable), function () {
                        var loader = {};
                ...
                $.when(saveDataTable(), loadDependencies(myChart)).then(function () {
                    try {
                        var Func = util.getObjectByPath(myChart),
                            chartFunc = new Func(document.getElementById(containerId)), ready = function () {
        ...
        queryOptions = $.extend(defaults.getQueryOptions(), queryOpt);
        chartOptions = $.extend(defaults.getChartOptions(), defaults.getChartSpecificOptions(chartFunction()), chartOpt);
        ...
        return {
            ...
            getDataTable: function (success, fail) {
                $.when(saveDataTable()).then(function () {
    ...
    S.container = function () {
        var util = S.util, loadDependencies = S.loader.loadDependencies, logger = S.logger, Query = S.Query,
            PREFIX = "data-sgvizler-", PREFIXCHART = PREFIX + "chart-options", VALUEASSIGN = "=", VALUESPLIT = "|",
            getQueryAttributes = function (elementID) {
                var i, len, queryOpt = {}, elmAttrs = document.getElementById(elementID).attributes;
                ...
                var i, j, ilen, jlen, options, assignment, path, o, chartOpt = {},
                ...
                var queryOptions = getQueryAttributes(elementID), chartOptions = getChartAttributes(elementID),
    ...
    S.form = function () {
        var util = S.util, prefixesSPARQL = S.namespace.prefixesSPARQL, registry = S.registry, Query = S.Query,
            idprefix = "sgvzlr_", idMainCon = idprefix + "mainCon", idFormCon = idprefix + "formCon",
            idChartCon = idprefix + "gchart", idQueryForm = idprefix + "formQuery", idQueryTxt = idprefix + "cQuery",
            idFormQuery = idprefix + "strQuery", idFormEndpointGrp = idprefix + "strEndpointGrp",
            idFormEndpoint = idprefix + "strEndpoint", idFormFormatGrp = idprefix + "btnFormatGrp",
            idFormFormat = idprefix + "btnFormat", idFormSizeGrp = idprefix + "strSizeGrp",
            idFormWidth = idprefix + "strWidth", idFormHeight = idprefix + "strHeight",
            idFormChartGrp = idprefix + "optChartGrp", idFormChart = idprefix + "optChart",
            idFormButtonGrp = idprefix + "btnSendGrp", idPrefixCon = idprefix + "cPrefix",
            idMessageCon = idprefix + "cMessage", idLogo = idprefix + "logo", idFooter = idprefix + "footer",
            html = function () {
                return {
                    heading: ["h1", null, ["Sgvizler"]],
                    logo: ["div", {id: idLogo}, [["a", {href: S.core.HOMEPAGE}, [["img", {
                        src: S.core.LOGOIMAGE,
                        alt: "Mr. Sgvizler"
                    }]]], "Mr. Sgvizler"]],
                    main: ["div", {id: idFormCon}, [["pre", {id: idPrefixCon}], ["textarea", {
                        id: idQueryTxt,
                        rows: "10",
                        cols: "80"
                    }], ["form", {id: idQueryForm, method: "get"}, [["p", null, [["input", {
                        id: idFormQuery,
                        type: "hidden",
                        name: "query",
                        value: ""
                    }], ["span", {id: idFormEndpointGrp}, [["label", {"for": idFormEndpoint}, ["Endpoint: "]], ["input", {
                        id: idFormEndpoint,
                        type: "text",
                        name: "endpoint",
                        size: "30"
                    }]]], ["span", {id: idFormFormatGrp}, [["label", {"for": idFormFormat}, ["Output format: "]], ["input", {
                        id: idFormFormat,
                        type: "radio",
                        ...
                        name: "endpoint_output",
                        value: "jsonp"
                    }], "jsonp"]], ["span", {id: idFormChartGrp}, [["label", {"for": idFormChart}, ["Chart type: "]], ["select", {
                        id: idFormChart,
                        name: "chart"
                    }]]], ["span", {id: idFormSizeGrp}, [["label", {"for": idFormWidth}, ["Width: "]], ["input", {
                        id: idFormWidth,
                        name: "width",
                        type: "text",
                        size: "3"
                    }], ["label", {"for": idFormHeight}, ["Height: "]], ["input", {
                        id: idFormHeight,
                        name: "height",
                        type: "text",
                        size: "3"
                    }]]], ["span", {id: idFormButtonGrp}, [["input", {
                        type: "button",
                        value: "Reset",
                        onclick: "sgvizler.formReset()"
                    }], ["input", {
                        type: "button",
                        value: "Go",
                        onclick: "sgvizler.formSubmit()"
                    }]]]]]]], ["div", {id: idMessageCon}]]],
                    chart: ["div", {id: idChartCon, style: "width:800px; height:400px;"}],
                    footer: ["div", {id: idFooter}, [["p", null, [["a", {href: S.core.HOMEPAGE}, ["Sgvizler"]], " version " + S.core.VERSION + " (c) 2011&ndash;2013 Martin G. Skj&#230;veland."]]]]
                }
            }(), permissible_urlparams = ["query", "endpoint", "endpoint_output", "chart", "width", "height", "ui"],
            isElement = function (elementID) {
                ...
                var i, j, ilen, jlen, charts = registry.chartFunctions().sort(), createOptGrp = function (name) {
                    return util.createHTMLElement("optgroup", {label: name + ".*"})
                }, libs = registry.chartModules(), group = {}, added = false;
                    ...
                    for (i = 0, ilen = charts.length; i < ilen; i += 1) {
                        ...
                        for (j = 0, jlen = libs.length; j < jlen; j += 1) {
                            if (util.startsWith(charts[i], libs[j])) {
                                $(group[libs[j]]).append($("<option/>").val(charts[i]).html(charts[i].replace(libs[j] + ".", "")));
                                added = true
                            }
                        }
                        if (!added) {
                            $("#" + idFormChart).append($("<option/>").val(charts[i]).html(charts[i]))
                        }
                    }
                }
            }, createUI = function (elementID, UItype) {
                var createHTMLElements = function (elementsArray) {
                    ...
                    if (elementID) {
                        ...
                        $("body").append(createHTMLElements(html.logo), createHTMLElements(html.heading), main, createHTMLElements(html.footer))
                    }
                    main.append(createHTMLElements(html.main), createHTMLElements(html.chart))
                };
                if (UItype === "result") {
                ...
                var urlParams = {}, e, r = /([^&=]+)=?([^&]*)/g, d = function (s) {
                    return decodeURIComponent(s.replace(/\+/g, " "))
                }, q = window.location.search.substring(1), params = urlparams || permissible_urlparams;
                ...
                var params = getUrlParams(), query = new Query({
                    query: params.query,
                    chart: params.chart,
        ...
        return {draw: draw, reset: reset, submit: submit}
    }();
    S.visualization = function () {
        var util = S.util, namespace = S.namespace, charts = S.charts, C = {}, modSC = "sgvizler.visualization";
        C.D3ForceGraph = charts.add(modSC, "D3ForceGraph", function (data, chartOpt) {
            var r, noRows = data.getNumberOfRows(), i, len, noColumns = data.getNumberOfColumns(),
                opt = $.extend({maxnodesize: 15, minnodesize: 2}, chartOpt), colors = d3.scale.category20(),
                w = chartOpt.width, h = chartOpt.height, isNumber = function (n) {
                    return !isNaN(parseFloat(n)) && isFinite(n)
                }, nodes = [], edges = [], t_color = {}, t_size = {}, t_maxnodesize = 0, source, target, nodesizeratio,
                color, size, vis, force, link, node, ticks;
            C.util.loadCSS();
            for (r = 0; r < noRows; r += 1) {
                ...
                if (source !== null && target !== null) {
                    edges.push({source: $.inArray(source, nodes), target: $.inArray(target, nodes)})
                }
            }
            if (t_maxnodesize === 0) {
                t_maxnodesize = 1
            ...
            for (i = 0, len = nodes.length; i < len; i += 1) {
                color = t_color[nodes[i]] !== undefined ? t_color[nodes[i]] : 1;
                size = isNumber(t_size[nodes[i]]) ? opt.minnodesize + t_size[nodes[i]] * nodesizeratio : opt.minnodesize;
                nodes[i] = {name: nodes[i], color: color, size: size}
            }
            $(this.container).empty();
            vis = d3.select(this.container).append("svg:svg").attr("width", w).attr("height", h).attr("pointer-events", "all").append("svg:g").call(d3.behavior.zoom().on("zoom", function () {
                vis.attr("transform", "translate(" + d3.event.translate + ")" + " scale(" + d3.event.scale + ")")
            })).append("svg:g");
            vis.append("svg:rect").attr("width", w).attr("height", h).attr("fill", "white");
            force = d3.layout.force().gravity(.05).distance(100).charge(-100).nodes(nodes).links(edges).size([w, h]).start();
            link = vis.selectAll("line.link").data(edges).enter().append("svg:line").attr("class", "link").attr("x1", function (d) {
                return d.source.x
            }).attr("y1", function (d) {
                return d.source.y
            }).attr("x2", function (d) {
            ...
            node = vis.selectAll("g.node").data(nodes).enter().append("svg:g").attr("class", "node").call(force.drag);
            node.append("svg:circle").style("fill", function (d) {
            ...
            force.on("tick", function () {
                ...
                if (ticks > 250) {
                    force.stop();
                    force.charge(0).linkStrength(0).linkDistance(0).gravity(0).start()
                }
        ...
        C.DefList = charts.add(modSC, "DefList", function (data, chartOpt) {
            var r, noRows = data.getNumberOfRows(), c, noColumns = data.getNumberOfColumns(), opt = $.extend({
                cellSep: " ",
                termPrefix: "",
                termPostfix: ":",
                definitionPrefix: "",
                definitionPostfix: ""
            }, chartOpt), list = "", term, definition;
            for (r = 0; r < noRows; r += 1) {
                term = "<dt>" + opt.termPrefix + C.util.linkify2String(data.getValue(r, 0)) + opt.termPostfix + "</dt>";
                definition = "<dd>" + opt.definitionPrefix;
                ...
                definition += opt.definitionPostfix + "</dd>";
                list += term + definition
            }
            $(this.container).empty().html(list);
            this.fireListener("ready")
        });
        C.DraculaGraph = charts.add(modSC, "DraculaGraph", function (data, chartOpt) {
            var row, numberOfRows = data.getNumberOfRows(), i, len, numberOfColumns = data.getNumberOfColumns(),
                opt = $.extend({
                    noderadius: .5,
                    nodefontsize: "10px",
                    nodeheight: 20,
                    ...
                    edgefontsize: "10px",
                    edgeseparator: ", "
                }, chartOpt), graph = new Dracula.Graph, layouter, renderer, edge, source, target, label,
                nodeRenderer = function (color, URL) {
                    return function (r, n) {
                        return r.set().push(r.rect(n.point[0], n.point[1], n.label.length * opt.nodepadding, opt.nodeheight).attr({
                            fill: color,
                            "stroke-width": opt.nodestrokewidth,
                            r: opt.nodecornerradius
                        })).push(r.text(n.point[0] + n.label.length * opt.nodepadding / 2, n.point[1] + opt.nodeheight / 2, n.label).attr({"font-size": opt.nodefontsize}).click(function () {
                            if (URL) {
                                window.open(namespace.unprefixify(URL))
                            }
                        }))
                    }
            ...
            for (row = 0; row < numberOfRows; row += 1) {
                ...
                if (source) {
                    addNode(source, data.getValue(row, 1) || source, numberOfColumns > 5 ? data.getValue(row, 5) : opt.nodecolor)
                }
                if (target) {
                    addNode(target, data.getValue(row, 3) || target, numberOfColumns > 6 ? data.getValue(row, 6) : opt.nodecolor)
                }
                if (source && target) {
                    label = "";
                    if (edges[source + target] !== undefined) {
                        label = edges[source + target].label
                    } else {
                    ...
                    edges[source + target] = {source: source, target: target, label: label}
                }
            }
            ...
            layouter.layout();
            $(this.container).empty();
            renderer = new Dracula.Graph.Renderer.Raphael(this.container, graph, opt.width, opt.height, {
                noderadius: opt.nodeheight * opt.noderadius
            });
            renderer.draw();
            this.fireListener("ready")
        }, {Dracula: "http://www.data2000.no/sgvizler/lib/raphael-dracula.min.js"});
        C.List = charts.add(modSC, "List", function (data, chartOpt) {
            var c, noColumns = data.getNumberOfColumns(), r, noRows = data.getNumberOfRows(),
        ...
        C.Map = charts.add(modSC, "Map", function (data, chartOpt) {
            var c, noColumns = data.getNumberOfColumns(), r, noRows = data.getNumberOfRows(), that = this,
                opt = $.extend({dataMode: "markers", showTip: true, useMapTypeControl: true}, chartOpt), chart, newData,
                newValue;
            C.util.loadCSS();
            if (noColumns > 3) {
                newData = data.clone();
            ...
            google.visualization.events.addListener(chart, "ready", function () {
                that.fireListener("ready")
            })
        }, {"google.visualization.Map": "map"});
        C.MapWKT = charts.add(modSC, "MapWKT", function (data, chartOpt) {
            var noColumns = data.getNumberOfColumns(), r, noRows = data.getNumberOfRows(), opt = $.extend({
                    zoom: 5,
                    centerLat: 62,
                    centerLong: 2,
                    geoDatumIn: "EPSG:4326",
                    geoDatumOut: "EPSG:4326"
                }, chartOpt), mapOptions = {
                    projection: opt.geoDatumOut,
                    layers: [new OpenLayers.Layer.OSM, new OpenLayers.Layer.Google("Google Physical", {type: google.maps.MapTypeId.TERRAIN}), new OpenLayers.Layer.Google("Google Streets", {numZoomLevels: 20}), new OpenLayers.Layer.Google("Google Hybrid", {
                    ...
                    controls: [new OpenLayers.Control.Navigation, new OpenLayers.Control.PanZoomBar, new OpenLayers.Control.LayerSwitcher({ascending: false}), new OpenLayers.Control.Permalink, new OpenLayers.Control.ScaleLine, new OpenLayers.Control.MousePosition, new OpenLayers.Control.OverviewMap, new OpenLayers.Control.KeyboardDefaults],
                    center: new OpenLayers.LonLat(opt.centerLong, opt.centerLat),
                    zoom: 5
                }, mapBounds, map = new OpenLayers.Map(this.container, mapOptions),
                formatterWKT = new OpenLayers.Format.WKT({
                    internalProjection: map.baseLayer.projection,
                    externalProjection: new OpenLayers.Projection(opt.geoDatumIn)
                }), resultLayer = new OpenLayers.Layer.Vector("Results", {
                    ...
                    eventListeners: {
                        featureselected: function (evt) {
                            var feature = evt.feature,
                                popup = new OpenLayers.Popup.FramedCloud("popup", OpenLayers.LonLat.fromString(feature.geometry.toShortString()), null, "<div style='font-size:.8em'>" + "<b>" + feature.attributes.name + "</b><br/>" + C.util.linkify2String(feature.attributes.uri) + "<br/>" + feature.attributes.description + "</div>", null, true);
                            feature.popup = popup;
                            map.addPopup(popup)
                ...
                }), selector = new OpenLayers.Control.SelectFeature(labelLayer, {click: true, autoActivate: true}),
                wktFeature, labelFeature, addWKT = function (layer, valueWKT) {
                    var features = formatterWKT.read(valueWKT), i;
                    if (features) {
            ...
            for (r = 0; r < noRows; r += 1) {
                ...
                labelFeature.attributes.name = noColumns > 1 && data.getValue(r, 1) ? data.getValue(r, 1) : "";
                labelFeature.attributes.uri = noColumns > 2 && data.getValue(r, 2) ? data.getValue(r, 2) : "";
                labelFeature.attributes.description = noColumns > 3 && data.getValue(r, 3) ? data.getValue(r, 3) : "";
        ...
        C.Table = charts.add(modSC, "Table", function (data, chartOpt) {
            var c, noColumns = data.getNumberOfColumns(), r, noRows = data.getNumberOfRows(),
                opt = $.extend({headings: true}, chartOpt), table, rows = [], cells = [];
            if (opt.headings) {
                for (c = 0; c < noColumns; c += 1) {
        ...
        C.Text = charts.add(modSC, "Text", function (data, chartOpt) {
            var c, noColumns = data.getNumberOfColumns(), r, noRows = data.getNumberOfRows(), opt = $.extend({
                cellSep: ", ",
                cellPrefix: "",
                cellPostfix: "",
                rowPrefix: "<p>",
                ...
                resultsPrefix: "<div>",
                resultsPostfix: "</div>"
            }, chartOpt), text = opt.resultsPrefix, row;
            for (r = 0; r < noRows; r += 1) {
                row = opt.rowPrefix;
                for (c = 0; c < noColumns; c += 1) {
            ...
            $(this.container).empty().html(text);
            this.fireListener("ready")
        });
        C.util = function () {
            var linkify = function (url, arraySyntax) {
                var prefixed = namespace.prefixify(url), base = namespace.getBaseURL, href = url, link, result;
                if (prefixed !== url) {
                    link = prefixed
    ...
    S.datatable = function () {
        var namespace = S.namespace, datatables = S.datatables, DT = {}, modST = "sgvizler.datatable";
        DT.pivot = datatables.add(modST, "pivot", function (dataTable) {
            var columns = dataTable.getDistinctValues(0), dataView = new google.visualization.DataView(dataTable),
                cary = columns.map(function (item) {
                    return {
                        type: "number", label: item, calc: function (dt, row) {
                            return dt.getValue(row, 0) === item ? dt.getValue(row, 2) : null
                        }
                    }
                }), rary = columns.map(function (item, index) {
                    return {column: index + 1, type: "number", label: item, aggregation: google.visualization.data.sum}
                });
            cary.unshift(1);
        ...
        DT.prefixify = datatables.add(modST, "prefixify", function (dataTable) {
            var c, clen = dataTable.getNumberOfColumns(), r, rlen = dataTable.getNumberOfRows();
            for (c = 0; c < clen; c += 1) {
                if (dataTable.getColumnType(c) === "string") {
                    for (r = 0; r < rlen; r += 1) {
                        dataTable.setValue(r, c, namespace.prefixify(dataTable.getValue(r, c)))
                    }
                }
            }
            return dataTable
